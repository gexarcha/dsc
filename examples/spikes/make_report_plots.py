...
for e in range(0,epochs,10)[::-1]:
	...
	# if not os.path.exists('{}montage_images/W_e_{:03}.eps'.format(outputdir, e)):
		...
		for h in range(H):
			...
			ax.set_title("$W_{"+str(h)+"}$",fontsize=10)
...
if series is not None and rseries is not None:
	...
	# for s in tqdm.tqdm(range(30000,T-l,l),'plotting'):
		...
		ax2=plt.subplot2grid((9,1),(2,0))
		ax3=plt.subplot2grid((9,1),(3,0))
		ax4=plt.subplot2grid((9,1),(4,0))
		...
		for ind in inds:
			...
			for b in rs[ind]:
				...
				if ind%3==2:
					height=np.max( W_all[-1,:,b] + O2*h)+2
					if rstssize[ind]==0:
					...
					height=np.max( W_all[-1,:,b] + O2*h)+2
					# height=np.max( W_all[-1,:,b] + O1 + O2*h)+2
					if rstssize[ind]==0:
					...
					height=np.max( W_all[-1,:,b] + O2*h)+2
					# height=np.max( W_all[-1,:,b] + 2*O1 + O2*h)+2
					if rstssize[ind]==0:
		...
		ax2.axis([s,s+l,np.min(W_all[-1])-3,np.max(np.abs(W_all[-1]))+(gamma-1)*O2+3],fontsize=10)
		ax2.set_yticks(tloc)
		ax2.set_yticklabels(tlab)
		ax2.tick_params(axis='both',labelsize=8)
		ax3.axis([s,s+l,np.min(W_all[-1])-3,np.max(np.abs(W_all[-1]))+(gamma-1)*O2+3],fontsize=10)
		ax3.set_yticks(tloc)
		ax3.set_yticklabels(tlab)
		ax3.tick_params(axis='both',labelsize=8)
		ax4.axis([s,s+l,np.min(W_all[-1])-3,np.max(np.abs(W_all[-1]))+(gamma-1)*O2+3],fontsize=10)
		ax4.set_yticks(tloc)
		...
		ax5 = plt.subplot2grid((9,1),(5,0),rowspan=2)
		ax5.plot(xdata,thisIC,label='IC')
		...
		ax5.axis([s,s+l,minic,maxic],fontsize=12)
		handles, labels = ax5.get_legend_handles_labels()
		lgd2 = ax5.legend(handles, labels, loc='upper right', bbox_to_anchor=(1.,1.),fontsize=9)
		ax5.tick_params(axis='both',labelsize=12)
		# ax5.grid('on')
		ax6=plt.subplot2grid((9,1),(7,0),rowspan=2)
		ax6.bar(these_lims,tmass,trssize)
		ax6.axis([s,s+l,0,gamma*np.max(states)*np.max(np.mean(np.abs(W_all[-1]),0))],fontsize=12)
