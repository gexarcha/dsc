...
globflags=dogparams[:,-1]<gabparams[:,-1]
# globflags=np.zeros((300),dtype='bool')
...
plt.clf()
plt.plot(sigma_all[1:301],label='$\sigma$')
...
plt.plot(sparsity[1:301],label='$\pi H$')
plt.savefig(outputdir+'sparsity.jpg')
plt.clf()

for e in tqdm.tqdm(list(range(epochs))[::-1],'epochs'):
    ...
    if not os.path.exists('{}montage_images/W_e_{:03}.jpg'.format(outputdir, e)):
        ...
        for h in range(H):
            ...
            ax.imshow(this_W,interpolation='nearest',vmin=minwl,vmax=maxwl,cmap='jet')
            ax.axis('off')
